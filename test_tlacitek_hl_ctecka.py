...
posledni_stisk_cas = 0  # Časovač pro ochranu tlačítek proti šumu
...
def otevri_knihu(nazev_souboru):
    ...
    if os.path.exists(cache_soubor):
        ...
        # Líné načítání těžkých knihoven pro zrychlení startu
...
def stisk_dalsi():
    print(">>> Fyzický dotyk detekován: DALŠÍ (Pin 21)")
    global aktualni_stranka, vybrana_kniha_index, prekreslit_displej, posledni_stisk_cas

    if probiha_vykreslovani:
        print("    ZAHRAZENO: Displej ještě neskončil překreslování!")
        return
    if time.time() - posledni_stisk_cas < 2.0:
        print("    ZAHRAZENO: Zmáčknuto moc rychle po sobě (ochrana proti šumu)!")
        return

    print("    POVOLENO: Jdu na další stranu / knihu.")
...
def stisk_predchozi():
    print(">>> Fyzický dotyk detekován: PŘEDCHOZÍ (Pin 26)")
    global aktualni_stranka, vybrana_kniha_index, prekreslit_displej, posledni_stisk_cas
    ...
    if probiha_vykreslovani:
        ...
        return
    if time.time() - posledni_stisk_cas < 2.0:
        print("    ZAHRAZENO: Zmáčknuto moc rychle po sobě!")
        return

    print("    POVOLENO: Jdu zpět.")
    posledni_stisk_cas = time.time()
...
def stisk_akce():
    print(">>> Fyzický dotyk detekován: AKCE (Pin 19)")
    ...
    print("    POVOLENO: Otvírám knihu / Vracím se do menu.")
    posledni_stisk_cas = time.time()
...
def stisk_akce_dlouhy():
    ...
    print(">>> Dlouhý stisk: Ukončuji program!")
    konec_programu = True
    ...
    try:
        while not konec_programu:
            if prekreslit_displej:
                        ...
                        for i, kniha in enumerate(seznam_knih):
                            ...
                            if i == vybrana_kniha_index:
                                draw_black.rectangle(
                                    (20, y_pozice - 2, 508, y_pozice + 42), fill=0
                                )
                                draw_black.text(
                                    (35, y_pozice),
                                    zobrazovany_nazev,
                ...
                epd.init()
                # Bleskové odeslání dat (obejití pomalého Waveshare ovladače)
                epd.display(
                    list(image_black_rotated.tobytes()),
                    list(image_red_rotated.tobytes()),
                )
                epd.sleep()
